...
class DiT1DforFlowDecoder(nn.Module):
    ...
    """
    def __init__(self, out_dim=263, embed_dim=384, c_in_dim=512, c_proj_dim=512, up_conv_c = "Repeat_and_stack_and_linear",num_heads=8, mlp_ratio=4, depth=6, max_seq_len=200, drop_out_prob=0.1,
                t_embedder="dit",
                pos_encoding=None,
        ...
        # self.cin_proj = nn.Linear(c_in_dim, c_proj_dim, bias=True) # it might be better to not project to a too samll dim
        ...
        elif t_embedder == "sin": # using dit is better!
        ...
        if up_conv_c == "Repeat_and_linear":
            self.cin_proj = nn.Linear(c_in_dim, c_proj_dim, bias=True)
    ...
    def forward(self, x, times, y, padding_mask=None, text_embedding=None):  
        ...
        if self.postional_encoding is not None:
            inputs = self.postional_encoding(inputs)
        ...
        for block in self.blocks:
            inputs = block(inputs, t, key_padding_mask=padding_mask, attn_mask=attn_mask)  # (B, T, embed_dim * 2)

        x = self.out_proj(inputs, t) # (B, T, out_dim )
        # x = inputs[:, :, :self.in_dim]
        return x
