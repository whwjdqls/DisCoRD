...
# NOTE 08/27 : code adopted from https://github.com/ChenFengYe/motion-latent-diffusion # 
# this is an implementation of a Transformer VAE
# Transformer VAE does not squash the input sequence, but uses learnable tokens to represent the sequence
# like the [CLASS] token in Vision Transformers
...
class ActorVae(nn.Module):
    ...
    def encode(
        ...
        dist,mu, logvar = self.encoder(features, lengths)
        if self.is_vae:
