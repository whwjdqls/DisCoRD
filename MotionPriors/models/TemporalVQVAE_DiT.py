...
from .base_models import LinearEmbedding, PositionalEncoding, PositionEmbedding
...
class TransformerEncoder(nn.Module):
    def __init__(self, 
        ...
        layers = [nn.Sequential( 
                    nn.Conv1d(in_dim,hidden_size,5,stride=2,padding=2,
...
class TransformerDecoder(nn.Module):
    def __init__(self,
        ...
        # for ConvTranspose1d as shown in the above link -> 
                    nn.ConvTranspose1d(in_dim,hidden_size,5,stride=2,padding=2,
                                        output_padding=1,
...
class TVQVAE_DiT(nn.Module):
    ...
    def forward(self, x_t, t, x_origin): # input (Bs, T, 263)
        loss, perplexity, vae_out = self.vae(x_origin)
        ## refine
        x_out = self.dit(x_t, t, vae_out)
        
        return x_out, (loss, perplexity, vae_out)
    
    def forward_with_cfg(self, x_t, t, x_origin, cfg_scale):
        loss, perplexity, vae_out = self.vae(x_origin)
        ## refine
        x_out = self.dit.forward_with_cfg(x_t, t, vae_out, cfg_scale)
...
class TVQVAE(nn.Module):
    ...
    def __init__(self, 
                ...
                temporal_bias = "alibi_future"):
        super().__init__()
        self.encoder = TransformerEncoder(in_dim=in_dim, 
                    hidden_size=hidden_size, 
                    dim_feedforward=dim_feedforward, 
                    num_attention_heads=num_attention_heads, 
                    num_hidden_layers=num_hidden_layers, 
        ...
        self.decoder = TransformerDecoder(in_dim=hidden_size, 
                    out_dim=in_dim, 
                    hidden_size=hidden_size, 
                    dim_feedforward=dim_feedforward, 
                    num_attention_heads=num_attention_heads, 
    ...
    def forward(self, x_origin): # input (Bs, T, 263)
        x_encoder = self.encoder(x_origin) # (Bs, T/q, embed_dim)
        x_encoder = self.pre_quant(x_encoder)# BS, T/q, embed_dim
        ## quantization
        ...
        return loss, perplexity, x_out

    def decode_with_emb(self, quant, t, c): #input (BS, T/q, embed_dim)
...
class ResTVQVAE_DiT(nn.Module):
    ...
    def __init__(self,
        ...
        self.encoder = Encoder(in_dim, embed_dim, quant_factor, stride_t, hidden_size, depth,
                            dilation_growth_rate, activation=activation, norm=norm)
        self.decoder = Decoder(in_dim, embed_dim, quant_factor, stride_t, hidden_size, depth,
                            dilation_growth_rate, activation=activation, norm=norm)
        ...
        self.DiT = DiTRefiner(
            ...
            mlp_ratio=dit_mlp_ratio,
            depth=dit_depth,
            drop_out_prob=dit_dropout_prob,
        )
    ...
    def forward(self, x_t, t, x_origin): # input (BS, T, 263)
        ...
        x_in = self.preprocess(x_origin) # (BS, 263, T)
        ...
        vae_out = x_out.clone()
        ## refine
        x_out = self.DiT(x_t, t, x_out)
        return x_out, (loss, perplexity, vae_out)
    
    def forward_with_cfg(self, x_t, t, x_origin, cfg_scale):
        x_in = self.preprocess(x_origin)
        x_encoder = self.encoder(x_in)
        ## quantization
        ...
        vae_out = x_out.clone()
        ## refine
        x_out = self.DiT.forward_with_cfg(x_t, t, x_out, cfg_scale)
        return x_out, (loss, perplexity, vae_out)
