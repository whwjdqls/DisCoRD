...
class ResidualVQ(nn.Module):
    ...
    def codebooks(self):
        ...
        codebooks = torch.stack(codebooks, dim = 0) # stack layers at the first dimension
    ...
    def get_codes_from_indices(self, indices): #indices shape 'b n q' # dequantize
        ...
        if quantize_dim < self.num_quantizers: # if we are using less quantizers, padd the quant dimension with -1s
            indices = F.pad(indices, (0, self.num_quantizers - quantize_dim), value = -1)

        # get ready for gathering
        # extend by batch
        codebooks = repeat(self.codebooks, 'q c d -> q b c d', b = batch) # repeat the codebook with batch to index them
        gather_indices = repeat(indices, 'b n q -> q b n d', d = codebooks.shape[-1]) 
    

        # take care of quantizer dropout

        mask = gather_indices == -1. # mask are true if gather indices are -1, that is dummy values
